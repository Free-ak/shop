...
<body>
			...
			<td width="900px" class="content">
				...
				$query = "
							SELECT 
								...
								CONCAT('<a href=\"$table.php?delete_id=', `$table`.id, '\" $delete_confirm>', 'удалить&nbsp;#', `$table`.id, '</a>') AS 'Удаление',
								CONCAT('<a href=\"$table.php?edit_id=', `$table`.id, '\">', 'редактировать&nbsp;#', `$table`.id, '</a>') AS 'Редактирование'
								FROM 
								`$table`
							LEFT JOIN 
					...
					<form name="form" action="<?php echo $table ?>.php" method="post">
						<table>
							...
							<tr>
								<td>№ заказа</td>
								<td>
									<input id="ord_id" name="ord_id" type="text" value="<?php if (!empty($ord_id))
										echo $ord_id; ?>">
								</td>
							</tr>
							...
							<tr>
								<td>Количество</td>
								<td>
									<input id="amount" name="amount" type="text" value="<?php if (!empty($amount))
										echo $amount; ?>">
								</td>
							</tr>
	...
	";
							...
							<tr>
								<td>Цена за 1ед. </td>
								<td>
									<input id="price" name="price" type="text" value="<?php if (!empty($price))
										echo $price; ?>">
								</td>
							</tr>
							...
							<tr>
								<td>Пользователь</td>
								<td>
									<input id="user_id" name="user_id" type="text" value="<?php if (!empty($user_id))
										echo $user_id; ?>">
								</td>
							</tr>
